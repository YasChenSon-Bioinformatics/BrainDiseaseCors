...
GDSl <- download_GDSs(skipv = c('not-GPL570'), suppress = FALSE)
...
plotDegCorMatrix <- function( Ml, GDSl, n = 400 ){
  ...
  for ( i in seq_along(ESETl) ){
    ...
    control_samplev <- ! topped[[i]]$dm[, grepl('dz_', colnames(topped[[i]]$dm) )]
    disease_signature_matrix[ , i ] <-
...
pdf('~/BrainDiseaseCors/caprice/EDA/13datasets-cors.1stEd.pdf', width=15,height=12)
for ( n in c(200, 300, 400, 500, 600, 1000) ){
